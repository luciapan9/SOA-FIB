...
void check_milestone3() {
  char buf[32];
  page_table_entry *SPT2 = (page_table_entry *)(init_task->dir_pages_baseAddr[1].bits.pbase_addr << 12);

  // 1. Intentamos escribir primera entrada
  SPT2[0].entry = 0xDEADB00F;
  if (SPT2[0].entry == 0xDEADB00F) printk("KernelHigh[0] write OK\n");
  else printk("KernelHigh[0] ERROR\n");
  SPT2[0].entry = 0;

  // 2. En una entrada random
  SPT2[511].entry = 0x12345678;
  if (SPT2[511].entry == 0x12345678) printk("KernelHigh[511] write OK\n");
  else printk("KernelHigh[511] ERROR\n");
  SPT2[511].entry = 0;

  // 3. En la última
  SPT2[1023].entry = 0xCAFEBABE;
  if (SPT2[1023].entry == 0xCAFEBABE) printk("KernelHigh[1023] write OK\n");
  else printk("KernelHigh[1023] ERROR\n");
  SPT2[1023].entry = 0;
  ...
  main(void)
{
 ...
 /* Initialize task 1 data */
  ...
  // ===== PROBAR MILESTONE 3 ===== (ampliar la SPT)
  check_milestone3();
