...
struct list_head blocked;//lista procesos bloqueados con sys_block
...
void init_idle (void) {
	...
	// Kernel, ahora dos paginas
	DirAddress[0] = init_task->dir_pages_baseAddr[0];
	DirAddress[1] = init_task->dir_pages_baseAddr[1];
	...
	// Process blocking / keyboard read
	idle_union->task.pending_unblocks = 0;
	idle_union->task.charsParaLeer = 0;
...
void init_task1(void) {
    ...
    set_kernel_pages(KernelAddress);	//MD* setemos las traducciones a las paginas que contienen el codigo kernel (caben en una unica tp igual que antes), simplemente ahora tenemos 2 paginas


	int KernelHigh = alloc_frame(); ;// MD* otra tp para las traduciones a los frames fisicos de 1024 a 2048
	page_table_entry* KernelHighAddress = (page_table_entry*)(KernelHigh << 12);
	clear_page_table(KernelHighAddress);
	...
	set_ss_pag(KernelAddress,KernelHigh,KernelHigh,0);// MD* añadimos la nueva tp de kernel
	...
	DirAddress[1].bits.pbase_addr = KernelHigh;
	...
	DirAddress[2].bits.pbase_addr = User;
	...
	// Process blocking / keyboard read
	task1_union->task.pending_unblocks = 0;
	task1_union->task.charsParaLeer = 0;
