...
void clock_routine(void) {
  ...
  /** 
  if (zeos_ticks % 100 == 0) {
    // Si el actual es INIT, saltamos a IDLE
    if (current() == init_task) {
      printk("\n[TICK] Cambiando de INIT a IDLE...\n");
      task_switch((union task_union*)idle_task);
    } 
    // Si el actual es IDLE, volvemos a INIT
    else if (current() == idle_task) {
      printk("\n[TICK] Cambiando de IDLE a INIT...\n");
