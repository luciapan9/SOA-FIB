...
int sys_fork(void) {
  // datos del padre
  page_table_entry *parent_SPT = get_PT(current());
  page_table_entry *parent_UPT = (page_table_entry *)(current()->dir_pages_baseAddr[1].bits.pbase_addr << 12);
  ...
  // Asignamos la entrada 1 su UPT
  child_dir[1].entry = 0;
  child_dir[1].bits.pbase_addr = user_frame;
  child_dir[1].bits.user = 1;
  child_dir[1].bits.rw = 1;
  child_dir[1].bits.present = 1;
  ...
  // a las @físicas del hijo. De esta manera podemos hacer el copy data.
  // Al acabar, hacer flush del TLB para eliminar mapeos temporales!!

  int pag_libre_parent = PAG_LOG_INIT_CODE + NUM_PAG_CODE; 

  for (int i = 0; i < NUM_PAG_DATA; ++i) {
    // Hijo: Mapeo de las @lógicas (iguales q el padre) a sus propias @físicas
    ...
    set_ss_pag(parent_UPT, (pag_libre_parent + i) - 1024, new_frames[i], 1); // -1024 pq me daba page fault 
  }
  ...
  copy_data((void *)(PAG_LOG_INIT_DATA << 12), (void *)(pag_libre_parent << 12), NUM_PAG_DATA * PAGE_SIZE);

  // 6.1 Eliminamos los mapeos temporales del padre
  for (int i = 0; i < NUM_PAG_DATA; i++) {
    del_ss_pag(parent_UPT, (pag_libre_parent + i) - 1024);
  }
  set_cr3(get_DIR(parent_struct));
...
void sys_exit(void) {
  ...
  page_table_entry *UPT = (page_table_entry *)(current_process->dir_pages_baseAddr[1].bits.pbase_addr << 12);
  ...
  int upt_frame = current_process->dir_pages_baseAddr[1].bits.pbase_addr;
